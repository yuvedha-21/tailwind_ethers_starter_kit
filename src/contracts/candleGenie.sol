...
contract CandleGeniePrediction is Ownable, Pausable, ReentrancyGuard 
{
    ...
    function _safeClaim(uint256[] calldata epochs) internal
    {
        ...
        require(amountToClaim > 0, "Not found any eligible reward or refunds for claim");
    ...
    function claimable(uint256 epoch, address user) public view returns (bool) 
    {
        ...
        if (!round.closed || bet.amount <= 0 || bet.claimed || round.lockPrice <= 0 || round.closePrice <= 0  || round.lockPrice == round.closePrice) 
        {
            return false;
        }
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) 
    {
        ...
        if (bet.amount <= 0 || bet.claimed || round.closeTimestamp <= 0 || block.timestamp < round.closeTimestamp + 5) 
        {
            return false;
        }
